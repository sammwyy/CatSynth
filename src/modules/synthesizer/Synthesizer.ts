...
import {
  SequencerEvent,
  EventType as SequencerEventType,
} from "./SequencerEvent";
...
function bindFunctions() {
  ...
  fluid_synth_error = _module.cwrap("fluid_synth_error", "string", ["number"]);
...
function waitForInitialized() {
  ...
  if (typeof AudioWorkletGlobalScope !== "undefined") {
    ...
    mod = Module;
    addOnPostRunFn =
      typeof addOnPostRun !== "undefined" ? addOnPostRun : undefined;
  }
  if (mod.calledRun) {
...
function setBoolValueForSettings(
  settings: SettingsId,
  name: string,
...
function setIntValueForSettings(
  settings: SettingsId,
  name: string,
...
function setNumValueForSettings(
  settings: SettingsId,
  name: string,
  value: number | undefined
) {
...
function setStrValueForSettings(
  settings: SettingsId,
  name: string,
...
function makeMIDIEventCallback(
  synth: Synthesizer,
  cb: HookMIDIEventCallback,
  param: any
) {
...
export class Synthesizer implements ISynthesizer {
  /** @internal */
  ...
  public isPlaying() {
    return (
      this._synth !== INVALID_POINTER && getActiveVoiceCount(this._synth) > 0
    );
  }
  ...
  public render(outBuffer: AudioBuffer | Float32Array[]) {
    const frameCount =
      "numberOfChannels" in outBuffer ? outBuffer.length : outBuffer[0].length;
    const channels =
