...
import {
  ISequencerEventData,
  rewriteEventDataImpl,
} from "./ISequencerEventData";
...
function bindFunctions() {
  ...
  fluid_sequencer_get_client_name = _module.cwrap(
...
export class Sequencer implements ISequencer {
  ...
  public getClientCount(): Promise<number> {
    return Promise.resolve<number>(
      _module._fluid_sequencer_count_clients(this._seq)
    );
  }
  ...
  public sendEventAt(
    ...
    if (ev !== null) {
      ...
      for (let i = 0; i < count; ++i) {
        ...
        _module._fluid_sequencer_send_at(
          this._seq,
          ev,
          tick,
          isAbsolute ? 1 : 0
  ...
  public sendEventToClientAt(
    ...
    const ev = makeEvent(event);
    if (ev !== null) {
      _module._fluid_event_set_dest(
        ev,
        clientId === -1 ? this._seqId : clientId
      );
  ...
  public removeAllEventsFromClient(clientId: number): void {
    _module._fluid_sequencer_remove_events(
      this._seq,
      -1,
      clientId === -1 ? this._seqId : clientId,
      -1
