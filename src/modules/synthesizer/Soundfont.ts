...
function bindFunctions() {
  ...
  fluid_sfont_get_name = _module.cwrap("fluid_sfont_get_name", "string", [
    "number",
  ]);
  fluid_preset_get_name = _module.cwrap("fluid_preset_get_name", "string", [
    "number",
  ]);
}
...
export class Soundfont {
  ...
  public static getSoundfontById(
    synth: Synthesizer,
    id: number
  ): Soundfont | null {
    bindFunctions();

    const sfont = _module._fluid_synth_get_sfont_by_id(
      synth.getRawSynthesizer(),
      id
    );
  ...
  public getPreset(bank: number, presetNum: number): Preset | null {
    const presetPtr: PresetPointer = _module._fluid_sfont_get_preset(
      this._ptr,
      bank,
      presetNum
    );
