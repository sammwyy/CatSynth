...
/** @internal */
export class WorkletSequencer implements ISequencer {
  /** @internal */
  ...
  public registerSequencerClientByName(
    ...
    return this.getRaw().then((seqPtr) =>
      MethodMessaging.postCallWithPromise<number>(
        this._messaging!,
  ...
  public getAllRegisteredClients(): Promise<ClientInfo[]> {
    return MethodMessaging.postCallWithPromise<ClientInfo[]>(
      this._messaging!,
  ...
  public sendEventAt(
    ...
    MethodMessaging.postCall(this._messaging!, "sendEventAt", [
      event,
      tick,
      isAbsolute,
    ]);
  }
  public sendEventToClientAt(
    clientId: number,
    event: SequencerEvent,
    tick: number,
    isAbsolute: boolean
  ): void {
    MethodMessaging.postCall(this._messaging!, "sendEventToClientAt", [
      clientId,
      event,
      tick,
      isAbsolute,
  ...
  public removeAllEventsFromClient(clientId: number): void {
    MethodMessaging.postCall(this._messaging!, "removeAllEventsFromClient", [
      clientId,
    ]);
  }

  public processSequencer(msecToProcess: number) {
    MethodMessaging.postCall(this._messaging!, "processSequencer", [
      msecToProcess,
    ]);
  }
}
