...
export class AudioWorkletNodeSynthesizer implements ISynthesizer {
  /** @internal */
  ...
  public setInterpolation(value: InterpolationValues, channel?: number) {
    MethodMessaging.postCall(this._messaging!, "setInterpolation", [
      value,
      channel,
    ]);
  }
  ...
  public setGain(gain: number) {
    this._gain = gain;
    MethodMessaging.postCallWithPromise<void>(this._messaging!, "setGain", [
      gain,
    ])
      .then(() => {
        return MethodMessaging.postCallWithPromise<number>(
          this._messaging!,
          "getGain",
          []
        );
  ...
  public setChannelType(channel: number, isDrum: boolean) {
    MethodMessaging.postCall(this._messaging!, "setChannelType", [
      channel,
      isDrum,
    ]);
  }
  ...
  public waitForVoicesStopped() {
    return MethodMessaging.postCallWithPromise<void>(
      this._messaging!,
  ...
  public getSFontObject(sfontId: number): Promise<WorkletSoundfont> {
    ...
    return MethodMessaging.postCallWithPromise<string>(
      this._messaging!,
      "getSFontObject",
      [channel.port2, sfontId]
    ).then((name) => {
  ...
  public setSFontBankOffset(id: number, offset: number) {
    MethodMessaging.postCall(this._messaging!, "setSFontBankOffset", [
      id,
      offset,
    ]);
  }
  ...
  public midiKeyPressure(chan: number, key: number, val: number) {
    MethodMessaging.postCall(this._messaging!, "midiKeyPressure", [
      chan,
      key,
      val,
    ]);
  ...
  public midiControl(chan: number, ctrl: number, val: number) {
    MethodMessaging.postCall(this._messaging!, "midiControl", [
      chan,
      ctrl,
      val,
    ]);
  ...
  public midiProgramChange(chan: number, prognum: number) {
    MethodMessaging.postCall(this._messaging!, "midiProgramChange", [
      chan,
      prognum,
    ]);
  }
  public midiChannelPressure(chan: number, val: number) {
    MethodMessaging.postCall(this._messaging!, "midiChannelPressure", [
      chan,
      val,
    ]);
  }
  ...
  public midiPitchWheelSensitivity(chan: number, val: number) {
    MethodMessaging.postCall(this._messaging!, "midiPitchWheelSensitivity", [
      chan,
      val,
    ]);
  }
  ...
  public midiSFontSelect(chan: number, sfontId: number) {
    MethodMessaging.postCall(this._messaging!, "midiSFontSelect", [
      chan,
      sfontId,
    ]);
  }
  public midiProgramSelect(
    ...
    MethodMessaging.postCall(this._messaging!, "midiProgramSelect", [
      chan,
      sfontId,
      bank,
      presetNum,
  ...
  public midiSetChannelType(chan: number, isDrum: boolean) {
    MethodMessaging.postCall(this._messaging!, "midiSetChannelType", [
      chan,
      isDrum,
    ]);
  }
   ...
   */
  public hookPlayerMIDIEventsByName(
   ...
   */
  public registerSequencerClientByName(
    seq: ISequencer,
    clientName: string,
    callbackName: string,
    param: number
