...
import { Constants, SynthesizerStatus } from "./AudioWorkletNodeSynthesizer";
...
export function registerAudioWorkletProcessor() {
  /**
  ...
  class Processor extends AudioWorkletProcessor {
    ...
    constructor(options: AudioWorkletNodeOptions) {
      ...
      this._messaging = initializeReturnPort(
        this.port,
        promiseInitialized,
        () => this.synth!,
        (data) => {
          switch (data.method) {
            ...
            case "hookPlayerMIDIEventsByName":
              {
                const r = this.doHookPlayerMIDIEvents(
                  data.args[0],
                  data.args[1]
                );
                if (r) {
    ...
    private doCreateSequencer(port: MessagePort): Promise<void> {
      return Synthesizer.createSequencer().then((seq) => {
        const messaging = initializeReturnPort(
          port,
          null,
          () => seq,
          (data) => {
            ...
            if (data.method === "getRaw") {
              postReturn(
                ...
                data.id,
                data.method,
                (seq as Sequencer).getRaw()
              );
              return true;
            } else if (data.method === "registerSequencerClientByName") {
              const r = this.doRegisterSequencerClient(
                seq as Sequencer,
                data.args[0],
                data.args[1],
                data.args[2]
      ...
      const messaging = initializeReturnPort(
        ...
        (data) => {
          if (data.method === "getPresetIterable") {
            postReturn(messaging, data.id, data.method, [
              ...sfont.getPresetIterable(),
            ]);
            return true;
          }
          return false;
    ...
    private doHookPlayerMIDIEvents(
    ...
    private doRegisterSequencerClient(
      seq: Sequencer,
      clientName: string,
      callbackName: string,
      param: number
